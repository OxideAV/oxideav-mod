...
- README: XM oracle-gate table (every rule the staged texts leave open, with its pinned behaviour), reworked E4x / E7x, `Lxy` and autovibrato bullets, fuzz-target table.
- Mixer depth: the XM and IT renders resolve per-voice routing once per tick block instead of per frame (XM: instrument / sample lookup + the `FinalPan` formula; IT: the sounding-voice list, the pan share and a frozen loop view `ItLoopView::frozen` that stops re-resolving the sustain / normal loop three times per frame). Output is byte-identical (PCM hash pinned across the change); measured on a 60 s synthetic render: XM 92 ms → 76 ms, IT at the 256-voice NNA ceiling 2.29 s → 1.48 s.
- Hidden `xm_writer` fixture assembler (header / packed patterns / instruments with the extended header / 40-byte sample headers / delta-coded 8- and 16-bit bodies) and `tests/xm_oracle_compare.rs`: black-box FastTracker 2 render gates against an installed command-line player (`openmpt123 --render` / `xmp -o`, PCM only; clean SKIP when absent) comparing per-row / per-tick pitch, level, balance and flow traces.
...
- IT packed patterns (`it::parse_patterns`): channel-marker / mask-byte walk with the per-channel previous-mask and last-note / instrument / volume / command memories, volume-column decoding (`ItVolumeColumn`), row-count clamp, truncation tolerance; `it::parse_module` bundle + `it::estimate_duration_micros`.
- IT playback engine (`it_player::ItPlayerState`): row/tick scheduler, sample mode and instrument mode (volume / panning / pitch envelopes with loop + sustain loop, fadeout, NNA virtual channels with cut / continue / note-off / fade, DCT / DCA duplicate checks, pitch-pan separation, random volume), linear (`2^(n/768)`) and Amiga (ST3 period) slide modes, sample vibrato, the `PitchTable` / `FineSineData` / `FineRampDownData` tables, and the effect set `Axx`..`Zxx` incl. every `Sxx` sub-command and the volume-column forms.
- IT framework registration: `it` container (probe on the `IMPM` magic, single-packet demuxer with title / message / tracker / sample-name / extra_info metadata and a duration estimate) and the `"it"` codec id (`CODEC_ID_IT_STR`, `decoder::make_it_decoder`) emitting interleaved S16 stereo at 44.1 kHz; hidden `it_writer` fixture assembler; `tests/it_smoke.rs` registry-path pipeline pin.
- `tests/it_oracle_compare.rs`: black-box render gates against an installed command-line IT player (`openmpt123 --render` / `xmp -o`, PCM only; clean SKIP when absent) — 18 fixtures pin the pitch table, linear slides, vibrato depth + phase, tremolo, tremor, volume / channel / global slides, envelopes + fadeout, pitch-envelope units (½ semitone), pan commands + pan-envelope scaling, NNA / DCT, retrig, arpeggio, sample vibrato, offsets, tempo + tempo slides, sample-mode note-off, `Sxx` memory and absolute mix level.
- Oracle batch 3 (25 fixtures total): Amiga-slides mode, Old Effects (period-domain vibrato sign + row-tick hold, tremor `+1`), order-flow timing (`SBx` / `SEx` / `S6x` / `SDx` / `SCx` / `Bxx` / `Cxx` / `+++`), `S7x` past-note controls, note-fade cells, instrument-only column, DCT sample / instrument, panbrello (256-entry table stepped by `x`, depth/32), note-off against a looping volume envelope.
- Unit pins for the text-pinned rules the oracle cannot gate: compatible-`Gxx` `NewC5/OldC5` retune + E/F↔G memory link + envelope retrigger, Old-Effects vibrato hold, panbrello period, `S00` memory, `S6x` tick delay, `T0x`/`T1x`, note-fade cells, `S70`/`S72`/`S73`, instrument-only column, pan-envelope edge scaling.
- `it_decode` fuzz target (parser + bounded player render) with three writer-built corpus seeds; 7.7M executions / 540 s clean at round 455.
- README: Impulse Tracker coverage section (format surface, player semantics, oracle gates, known gaps).
...
- XM `9xx` sample offset at or past the sample's end plays nothing at all — also on looped samples (the manual's "9FF -> Nothing!"). Oracle-pinned (`offset` gate; `scale_linear` gate now pins the relative level only, the v2.04 formula's `Scale` factor being unspecified).
- Shared mixer ping-pong loops (MOD-family `MixerVoice`, used by STM / XM / IT): the loop now plays as a `2 × span` virtual waveform — body, then body mirrored with the end frames doubled at each turn — so a `span`-frame bidirectional loop repeats every `2 × span` frames; reflecting around `loop_end - 1` lost two frames per cycle and played ping-pong samples ~3% sharp. Oracle-pinned (XM `loops` gate: 16-bit and 8-bit ping-pong, loop length 0 = one-shot, tail past `loop_end` discarded); the IT oracle battery is unchanged.
- XM keymap edges: a sample-map entry past the instrument's sample count plays sample 0 (not the last sample), and a tone-portamento note keeps the playing sample — its target is computed with the current sample's relative note and finetune even when the keymap routes that note elsewhere. Oracle-pinned (`keymap` gate: split routing, out-of-range entry, instrument-only column, `3xx` across the split).
- XM order flow: `Bxx` / `Dxx` on any channel of a row win over a pattern-loop jump on that row; a `Dxx` row past the next pattern's length starts it at row 0; `F00` is ignored (it is not an end-of-song marker). Pattern loops follow FT2's state machine: every loop jump latches the loop-start row as the next pattern's start row and a finished loop leaves it there (the following pattern starts on that row; the latch is consumed by the advance), while a further `E6x` after a loop ran out in the same pattern (no new `E60`) plays `x - 1` passes and then breaks to the next pattern's row 0 without consuming the latch. Oracle-pinned (`loop_then_jump`, `jump_then_loop`, `loop_then_break`, `break_then_loop`, `loop_plain`, `jump_break_combos`, `speed_zero` gates + twelve loop-shape probes).
- XM volume-column slides (`+x` / `-x`, `Ux` / `Dx`): a zero amount is a no-op (the column has no memory of its own), while a non-zero `+x` / `-x` seeds the `Axy` memory so a later `A00` keeps sliding at the column's rate. Oracle-pinned (`vol_column_slides` gate).
...
- XM envelope tick order and note delay: the envelope position advances before each read except on the tick that set it (a trigger reads position 0, `Lxx` reads `xx`, both on that very tick); a key-off tick still counts as held so a sustained cursor moves on the tick after; the segment cursor is gone (positions are evaluated directly). `EDx` now defers the whole cell — instrument column re-read, volume column, pitch and key-off all land on tick x through the same `fire_cell` path as a tick-0 cell, a delay of `speed` or more never fires, and the delayed fire runs ahead of that tick's envelope read. Oracle-pinned (`env_sustain_vs_loop`, `vol_env_delayed`, `pan_env_autovib_delayed`, `note_delay`, `envpos` gates).
- XM instrument autovibrato: the 256-step position advances by `rate` before each read (every tick, including tick 0); the offset is `wave × depth` linear period units with the sine and square starting negative (pitch rises first); the instrument type byte maps 1 → square and 2 → ramp (the in-tree note's assumption that the `E4x` numbering carries over does not hold); the sweep scales the depth linearly over `sweep` ticks. Oracle-pinned (`autovib_shapes`, `autovib_depth` gates); type values ≥ 3 stay unpinned.
- XM vibrato / tremolo LFO cadence and scale: the 64-step position advances by `speed` per tick (not `speed × 4`), so a full cycle is `64 / speed` effect ticks; vibrato depth is `lfo × depth / 16` period units (±119 at depth 15) and tremolo `lfo × depth / 32` volume units; the `E4x` / `E7x` ramp shape (1) starts at 0, rises to +124 at the half cycle and wraps to -128. FT2's channel LFO is now its own `ft2_lfo` helper (the shared ProTracker helper stays for STM). Oracle-pinned (`vibrato`, `vibrato_depth`, `tremolo` gates; the tick gate accepts the oracle's two-tick volume ramp).
- XM `E5x` set finetune: the nibble is an unsigned offset from 8 on the signed-byte scale (`E58` = 0, `E50` = -128, `E5F` = +112), not a ProTracker signed nibble. Oracle-pinned (`tuning_linear` / `tuning_amiga` gates: sample finetune + relative note + `E5x` across both frequency tables).
